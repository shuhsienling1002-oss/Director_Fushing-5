...
def calculate_readiness(vf, hr, bp_sys, body_age, actual_age, social_mode, micro_workouts, water_intake, water_goal, no_alcohol):
    ...
    # 🍷 酒精與社交邏輯博弈 [cite: 38, 41]
    if social_mode:
        base_score -= 20
        if no_alcohol:
            base_score += 20  # 社交卓越獎勵：滴酒未沾抵銷負載
            
    if no_alcohol:
        base_score += 10      # 肝臟負熵紅利
...
# --- 📥 數據輸入 ---
with st.expander("📥 點此更新今日最新生理數值", expanded=False):
...
with col1:
    st.metric("代謝綜合評分", f"{st.session_state.readiness_score}%", delta_color="normal" if st.session_state.readiness_score >= 70 else "inverse")
with col2:
...
with col3:
    ...
    st.metric("代謝老化指標", f"{st.session_state.metrics['body_age']} 歲", f"老化 +{age_gap} 歲" if age_gap > 0 else f"年輕 {-age_gap} 歲", delta_color="inverse")
...
with col_h2:
    st.subheader("🚫 酒精防禦")
    # ✅ 修正後的無酒精開關
    is_sober = st.checkbox("🍺 今日沒喝酒 (代謝修復中)", value=st.session_state.no_alcohol)
...
if st.button("💾 儲存今日完整日誌"):
    ...
    ''', (
        today_str, 
        st.session_state.metrics['actual_age'], 
        st.session_state.metrics['body_age'], 
        ...
        st.session_state.micro_workouts, 
        st.session_state.water_intake, 
        st.session_state.no_alcohol
    ))
    conn.commit()
    conn.close()
    st.success("✅ 數據已安全存入本地資料庫！")

# --- 📊 歷史數據展示 ---
st.subheader("📊 近期代謝趨勢")
try:
    history_df = pd.read_sql_query("SELECT * FROM health_logs ORDER BY date DESC LIMIT 5", sqlite3.connect('fuxing_guardian_v4.db'))
    st.dataframe(history_df[['date', 'visceral_fat', 'readiness_score', 'no_alcohol']], use_container_width=True)
except:
    st.info("尚無歷史數據。")
